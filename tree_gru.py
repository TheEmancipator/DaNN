...
class ChildSumTreeGRU(tree_rnn.TreeRNN):
    ...
    def create_recursive_unit_disc_type(self, is_first):
        if is_first:
            self.W_z = theano.shared(self.init_matrix([self.hidden_dim+self.discourse_emb_dim, self.emb_dim]))
            self.U_z = theano.shared(self.init_matrix([self.hidden_dim+self.discourse_emb_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.W_r = theano.shared(self.init_matrix([self.hidden_dim+self.discourse_emb_dim, self.emb_dim]))
            self.U_r = theano.shared(self.init_matrix([self.hidden_dim+self.discourse_emb_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.W_h = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_h = theano.shared(self.init_matrix([self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.params.extend([
                self.W_z, self.U_z,
                self.W_r, self.U_r,
                self.W_h, self.U_h])
        ...
        def unit(parent_x, parent_discourse_x, child_h, child_exists):
            z = _softmax(
                ...
                child_exists, add_one=True)
            # z:
            # [1, hidden_dim+discourse_emb_dim]
            # [degree, hidden_dim+discourse_emb_dim] x [hidden_dim+discourse_emb_dim, hidden_dim+discourse_emb_dim] = [degree, hidden_dim+discourse_emb_dim]

            r = _softmax(
                (T.dot(self.W_r, parent_x).dimshuffle('x', 0) +
                 T.dot(child_h, self.U_r.T)),
                child_exists, add_one=False)
            # r: [degree, hidden_dim+discourse_emb_dim]

            h_hat = T.tanh(T.dot(self.W_h, parent_x) +
                           T.dot(self.U_h, T.sum(r * child_h, axis=0)))
            # h_hat:
            # [hidden_dim, 1]
            # [hidden_dim, hidden_dim+discourse_emb_dim] x [hidden_dim+discourse_emb_dim] = [hidden_dim]

            h_hat = T.concatenate([h_hat, parent_discourse_x]) #TODO: is it right?
            h = (1 - T.sum(z, axis=0)) * h_hat + T.sum(z * child_h, axis=0)
    ...
    def create_leaf_unit_disc_type(self):
        dummy = 0 * theano.shared(self.init_vector([self.degree, self.hidden_dim+self.discourse_emb_dim]))
...
class NaryTreeGRU(ChildSumTreeGRU):
    ...
    def create_recursive_unit_disc_type(self, is_first):
        ...
        if is_first:
            self.W_z = theano.shared(self.init_matrix([self.hidden_dim+self.discourse_emb_dim, self.emb_dim]))
            self.U_z = theano.shared(self.init_matrix(
                [self.degree, self.hidden_dim+self.discourse_emb_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.W_r = theano.shared(self.init_matrix([self.hidden_dim+self.discourse_emb_dim, self.emb_dim]))
            self.U_r = theano.shared(self.init_matrix(
                [self.degree, self.hidden_dim+self.discourse_emb_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.W_h = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
        ...
        def unit(parent_x, parent_discourse_x, child_h, child_exists):
            ...
            h_hat = T.tanh(T.dot(self.W_h, parent_x) +
                           T.dot(self.U_h, T.sum(r * child_h, axis=0)))
            h_hat = T.concatenate([h_hat, parent_discourse_x])
            h = (1 - T.sum(z, axis=0)) * h_hat + T.sum(z * child_h, axis=0)
            return h
