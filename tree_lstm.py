...
class ChildSumTreeLSTM(tree_rnn.TreeRNN):
    def create_recursive_unit(self, is_first):
        if is_first:
            ...
            self.params.extend([ #TODO: exp3 - Regularization (recurrent layer)
    ...
    def create_recursive_unit_disc_type(self, is_first):
        if is_first:
            self.W_i = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_i = theano.shared(self.init_matrix([self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.b_i = theano.shared(self.init_vector([self.hidden_dim]))
            self.W_f = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_f = theano.shared(self.init_matrix([self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.b_f = theano.shared(self.init_vector([self.hidden_dim]))
            self.W_o = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_o = theano.shared(self.init_matrix([self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.b_o = theano.shared(self.init_vector([self.hidden_dim]))
            self.W_u = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_u = theano.shared(self.init_matrix([self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.b_u = theano.shared(self.init_vector([self.hidden_dim]))
            self.params.extend([ #TODO: exp3 - Regularization (recurrent layer)
                self.W_i, self.U_i, self.b_i,
                self.W_f, self.U_f, self.b_f,
        ...
        def unit(parent_x, parent_discourse_x, child_h, child_c, child_exists):
            h_tilde = T.sum(child_h, axis=0)
            i = T.nnet.sigmoid(T.dot(self.W_i, parent_x) + T.dot(self.U_i, h_tilde) + self.b_i)
            o = T.nnet.sigmoid(T.dot(self.W_o, parent_x) + T.dot(self.U_o, h_tilde) + self.b_o)
            u = T.tanh(T.dot(self.W_u, parent_x) + T.dot(self.U_u, h_tilde) + self.b_u)
    ...
    def create_leaf_unit_disc_type(self):
        dummy_h = 0 * theano.shared(self.init_vector([self.degree, self.hidden_dim+self.discourse_emb_dim]))
        dummy_c = 0 * theano.shared(self.init_vector([self.degree, self.hidden_dim]))
    ...
    def compute_tree_disc_type(self, emb_x, discourse_emb_x, tree, is_first=True):
        ...
        # use recurrence to compute internal node hidden states
        def _recurrence(cur_emb, cur_discourse_emb, node_info, t, node_h, node_c, last_h):
            child_exists = node_info > -1
            offset = num_leaves * int(self.irregular_tree) - child_exists * t
            child_h = node_h[node_info + offset] * child_exists.dimshuffle(0, 'x')
            child_c = node_c[node_info + offset] * child_exists.dimshuffle(0, 'x')
            parent_h, parent_c = self.recursive_unit(cur_emb, cur_discourse_emb, child_h, child_c, child_exists)
            node_h = T.concatenate([node_h,
...
class NaryTreeLSTM(ChildSumTreeLSTM):
    ...
    def create_recursive_unit_disc_type(self, is_first):
        ...
        if is_first:
            self.W_i = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_i = theano.shared(self.init_matrix(
                [self.degree, self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.b_i = theano.shared(self.init_vector([self.hidden_dim]))
            self.W_f = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_f = theano.shared(self.init_matrix(
                [self.degree, self.degree, self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
            self.b_f = theano.shared(self.init_vector([self.hidden_dim]))
            self.W_o = theano.shared(self.init_matrix([self.hidden_dim, self.emb_dim]))
            self.U_o = theano.shared(self.init_matrix(
                [self.degree, self.hidden_dim, self.hidden_dim+self.discourse_emb_dim]))
        ...
        def unit(parent_x, parent_discourse_x, child_h, child_c, child_exists):
            ...
            # T.dot(Ui, h) : [hidden_dim]
            # >>map>> h_i [degree, hidden_dim]
            # T.dot(Uo, h) : [hidden_dim]
            # >>map>> h_o [degree, hidden_dim]
            # T.dot(Uu, h) : [hidden_dim]
            # >>map>> h_u [degree, hidden_dim]
            ...
            def _sub_f(U):
                sub_h_f, _ = theano.map(
                    fn=lambda sub_U, h, exists:
                        exists * T.dot(sub_U, h),
                    sequences=[U, child_h, child_exists])
                return sub_h_f.sum(axis=0)
            ...
            # T.dot(sub_U, h) : [hidden_dim]
            # >>map>> [degree, hidden_dim]
            # >>map>> [degree, degree, hidden_dim]
            ...
            # T.dot(self.W_f, parent_x).dimshuffle : [hidden_dim x 1]
            # h_f : [degree, degree, hidden_dim]
            # b_f.dimshuffle : [hidden_dim x 1]
